...
# Verify that GRUB 2 is configured to disable merging of slab caches.
GROUP_ID="V-230277"
...
if [[ "$1" == "--apply-script" ]]; then
  ...
  if ! grub2-editenv list | grep -q 'slab_nomerge'; then
      echo "          -> Applying fix: Running 'grubby' to add 'slab_nomerge' to kernel arguments."
      grubby --update-kernel=ALL --args="slab_nomerge"
      changes_made=true
  fi
  ...
  if grep -q "^GRUB_CMDLINE_LINUX=" "$default_grub_file"; then
      if ! grep "^GRUB_CMDLINE_LINUX=" "$default_grub_file" | grep -q "slab_nomerge"; then
          echo "          -> Applying fix: Adding 'slab_nomerge' to GRUB_CMDLINE_LINUX in ${default_grub_file}."
          sed -i 's/\(GRUB_CMDLINE_LINUX=".*\)"/\1 slab_nomerge"/' "$default_grub_file"
          changes_made=true
      fi
  else
      echo "          -> Applying fix: Adding 'GRUB_CMDLINE_LINUX=\"slab_nomerge\"' to ${default_grub_file}."
      echo "GRUB_CMDLINE_LINUX=\"slab_nomerge\"" >> "$default_grub_file"
      changes_made=true
  fi
  ...
  if ! echo "$kernel_opts" | grep -q "slab_nomerge"; then
    is_compliant=0
    findings_list+=("-> Finding: Slab cache merging is not disabled ('slab_nomerge') in the running configuration.")
  fi
  ...
  if [[ -f "$default_grub_file" ]]; then
      ...
      if ! echo "$persistent_opts" | grep -q "slab_nomerge"; then
          is_compliant=0
          findings_list+=("-> Finding: 'slab_nomerge' is not set in '${default_grub_file}' to persist on kernel updates.")
      fi
  else
      is_compliant=0
