...
# Verify RHEL 8 enables kernel page-table isolation.
GROUP_ID="V-230491"
...
if [[ "$1" == "--apply-script" ]]; then
  ...
  if ! grub2-editenv list | grep -q 'pti=on'; then
  ...
  if grep -q "^GRUB_CMDLINE_LINUX=" "$default_grub_file"; then
      # Line exists, check if pti=on is present
      if ! grep "^GRUB_CMDLINE_LINUX=" "$default_grub_file" | grep -q "pti=on"; then
          echo "          -> Applying fix: Adding 'pti=on' to GRUB_CMDLINE_LINUX in ${default_grub_file}."
          sed -i 's/\(GRUB_CMDLINE_LINUX=".*\)"/\1 pti=on"/' "$default_grub_file"
          changes_made=true
      fi
  else
      # Line does not exist, add it
      echo "          -> Applying fix: Adding 'GRUB_CMDLINE_LINUX=\"pti=on\"' to ${default_grub_file}."
      echo "GRUB_CMDLINE_LINUX=\"pti=on\"" >> "$default_grub_file"
      changes_made=true
  fi
  ...
  if ! echo "$kernel_opts" | grep -q "pti=on"; then
    is_compliant=0
    findings_list+=("-> Finding: Kernel page-table isolation ('pti=on') is not enabled in the running configuration.")
  fi
  ...
  if [[ -f "$default_grub_file" ]]; then
      ...
      if ! echo "$persistent_opts" | grep -q "pti=on"; then
          is_compliant=0
          findings_list+=("-> Finding: Kernel page-table isolation ('pti=on') is not set in '${default_grub_file}' to persist on kernel updates.")
      fi
  else
      is_compliant=0
