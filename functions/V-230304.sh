...
# Verify file systems for removable media are mounted with the "noexec" option.
GROUP_ID="V-230304"
...
if [[ "$1" == "--apply-script" ]]; then
  ...
  echo "          ->    add the 'noexec' option to the mount options (the 4th column)."
  echo "          ->    Example: UUID=... /mnt/usb vfat noauto,owner,noexec,nosuid 0 0"
      ...
      while IFS= read -r line; do
          ...
          if echo "$fs_type" | grep -qE "$removable_fs_types" || echo "$mount_options" | grep -q "noauto"; then
              ...
              if ! echo "$mount_options" | grep -q "noexec"; then
                  is_compliant=0
                  findings_list+=("-> Finding: Removable media entry does not have the 'noexec' mount option:")
                  findings_list+=("->   - ${line}")
              fi
          fi
      done <<< "$fstab_entries"
