...
if [[ "$1" == "--apply-script" ]]; then
  ...
  echo "          -> The AIDE configuration file must be edited manually to ensure the 'acl' check"
  ...
  echo "          ->    ensure '+acl' is part of the definition."
  echo "          ->    Example: NORMAL = p+i+n+u+g+s+m+c+md5+acl"
  ...
  if [[ -z "$aide_conf_path" ]]; then
      ...
      # Check 3: Analyze aide.conf for 'acl' usage
      # Get all rule definitions that DO NOT include 'acl'.
      # These are lines like 'RULENAME = ...'
      rules_without_acl=$(grep '^[A-Za-z0-9_]\+ \?=' "$aide_conf_path" | grep -v 'acl' | awk '{print $1}')

      # Now check if any of these non-compliant rules are actually used.
      for rule in $rules_without_acl; do
          # Check for selection lines (start with /) that use this specific rule.
          # We use -w for a whole word match to avoid partial matches (e.g., 'NORMAL' in 'NORMAL_PLUS').
          if grep -q -E "^/.*[[:space:]]+([^#]*\b${rule}\b.*)" "$aide_conf_path"; then
              is_compliant=0
              findings_list+=("-> Finding: Rule '${rule}' is used but does not check for ACLs.")
          fi
      done
      
      if [[ $is_compliant -eq 1 ]]; then
         check_results+=("->   ACL Check: All applied rules appear to include the 'acl' directive.")
      fi
  fi
